...
class ImproperNeutralizationOfElementsInDataQueryCompliant extends HttpServlet {

    // {fact rule=improper-neutralization-of-elements-in-data-query@v1.0 defects=0}
    @throws[IOException]
    override def compliant(request: HttpServletRequest, response: HttpServletResponse): Unit = {
        try {
            ...
            val query = "select * from invoices where customerID = 123"
            // Compliant: No untrusted input is used in the query.
